...
def _imagine(state, z_embedding, initial_states, action, rng, model_seq):
    variables = {'params': state.params, **state.model_state}
    rng, new_rng = jax.random.split(rng)
    out, _ = model_seq.apply(variables,
                             z_embedding, initial_states, action,
...
def sample(model_par, state, video, actions, action_conditioned, open_loop_ctx,
           p_observe, p_imagine, p_encode, p_decode, eval_seq_len,
    ...
    encodings = video[:, :, :num_input_frames]
    ...
    itr = list(range(num_input_frames, eval_seq_len))
    for i in tqdm(itr):
    ...
    return samples, video[:, :, :eval_seq_len]
