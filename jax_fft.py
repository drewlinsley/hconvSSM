...
def fft_conv(
    ...
    if ss is not None:
        signal_fr = jnp.fft.fftn(signal, (ss, ss), axes=tuple(range(2, signal.ndim)))
        kernel_fr = jnp.fft.fftn(padded_kernel, (ss, ss), axes=tuple(range(2, signal.ndim)))
    else:
    ...
    if ss is not None:
        output = jnp.fft.ifftn(output_fr, (ss, ss), axes=tuple(range(2, signal.ndim)))
    else:
        output = jnp.fft.ifftn(output_fr, axes=tuple(range(2, signal.ndim)))

    # Remove extra padded values
    crop_slices = [slice(None), slice(None)] + [
        slice(0, (signal_size[i] - kernel.shape[i] + 1), stride_[i - 2])
        for i in range(2, signal.ndim)
    ]
    # output = output[crop_slices[0], crop_slices[1], crop_slices[2], crop_slices[3]]
...
def _assert_almost_equal(x: jnp.ndarray, y: jnp.ndarray) -> bool:
    abs_error = jnp.abs(x - y)
    assert abs_error.mean() < 5e-5
...
def test_fft_conv_functional(
    ...
    signal = jax.random.normal(key, (batch_size, in_channels, *dims), dtype=dtype)
    kwargs = dict(
        # bias=jax.random.normal(key, (1, out_channels), dtype=dtype) if bias else None,
        padding=padding,
        stride=stride,
        dilation=dilation,
        groups=groups,
    ...
    w0 = jax.random.normal(key, (out_channels, in_channels // groups, *kernel_size), dtype=dtype)
    w1 = w0.copy()

    b0 = jax.random.normal(key, (1, out_channels), dtype=dtype) if bias else None
    b1 = b0.copy() if bias else None
    ...
    y1 = jax.lax.conv_general_dilated(signal, w1, window_strides=[stride, stride], padding=padding, dimension_numbers=('NCHW', 'OIHW', 'NCHW'), feature_group_count=groups) + b1[..., None, None]
    print((y0 - y1).sum())
    _assert_almost_equal(y0, y1)

# test_fft_conv_functional(3, 10, 3, 3//2, 1, 1, 1, True, 2, 10)
